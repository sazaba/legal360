...
const FormularioPlanes = () => {
    ...
    return (
        // lg:h-screen obliga a que todo quepa en el monitor sin scroll
        <section className="relative w-full min-h-[100svh] lg:h-screen flex flex-col lg:flex-row bg-[#f8fafc] overflow-hidden isolate">
            
            {/* ================= MITAD IZQUIERDA: FORMULARIO ================= */}
            <div className="w-full lg:w-[45%] xl:w-5/12 px-6 sm:px-12 lg:px-16 py-10 lg:py-0 flex flex-col justify-center">
                <div className="max-w-xl mx-auto w-full">
                    <div className="mb-6 lg:mb-4 text-center lg:text-left">
                        <h2 className="text-3xl lg:text-3xl xl:text-4xl font-extrabold text-[#001e33] font-montserrat mb-3 lg:mb-2 tracking-tight">
                            Agenda tu <br className="hidden lg:block"/>
                            <span className="text-[#001e33]">
                                Diagnóstico Gratuito
                            </span>
                        </h2>
                        <p className="text-sm lg:text-[13px] xl:text-base text-gray-500 font-roboto leading-relaxed lg:leading-snug">
                            Si usted es una persona jurídica o natural y requiere conocer cuál es su estado jurídico en materia derecho laboral y comercial, diligencie la siguiente información.
                        </p>
                    </div>

                    {/* gap-3 en desktop para compactar */}
                    <form onSubmit={handleSubmit} className="flex flex-col gap-3 lg:gap-2.5 xl:gap-4 text-[#001e33] text-sm font-roboto">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-3">
                            <input id="nombre" name="nombre" value={formData.nombre} onChange={handleChange} required placeholder="Nombre*" 
                                className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] transition-all placeholder-gray-400" />
                            <input id="apellido" name="apellido" value={formData.apellido} onChange={handleChange} required placeholder="Apellido*" 
                                className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] transition-all placeholder-gray-400" />
                        </div>
                        ...
                        <div className="flex gap-3">
                            <select id="pais" name="pais" value={formData.pais} onChange={handleChange} required 
                                className="w-1/3 bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] cursor-pointer text-gray-600">
                                <option value="Col">Col (+57)</option>
                            </select>
                            <input id="telefono" name="telefono" value={formData.telefono} onChange={handleChange} required placeholder="+57" 
                                className="w-2/3 bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] transition-all placeholder-gray-400" />
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-3">
                            <select id="cargo" name="cargo" value={formData.cargo} onChange={handleChange} required 
                                className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] cursor-pointer text-gray-600">
                                <option value="">Cargo*</option>
                                <option value="Gerente">Gerente / Socio</option>
                                <option value="Administrador">Administrador</option>
                                <option value="RRHH">Recursos Humanos</option>
                        ...
                        <textarea id="mensaje" name="mensaje" value={formData.mensaje} onChange={handleChange} rows="2" placeholder="Mensaje o consulta" 
                            className="w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 py-2.5 lg:py-2 xl:py-3 outline-none focus:ring-2 focus:ring-[#d4af37] resize-none placeholder-gray-400 text-xs xl:text-sm" />

                        <div className="bg-white border border-gray-200 rounded-xl p-4 lg:p-3 xl:p-5 space-y-3 lg:space-y-2 shadow-sm">
                            <p className="text-[10px] xl:text-[11px] text-gray-500 text-justify leading-tight">
                                En virtud de lo anterior, autorizo o no autorizo a LEGAL 360 S.A.S., para que realice tratamiento de mis datos personales y emita respuesta a inquietudes, envíe publicidad, comunicaciones, promociones, invitaciones a eventos, noticias, encuestas y cualquier información comercial a través de este medio.
                            </p>
                            
                            <div className="flex items-center gap-6 font-medium text-[#001e33] text-[11px] xl:text-xs">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="autorizacion" value="si" checked={formData.autorizacion === 'si'} onChange={handleChange} required 
                        ...
                        <button 
                            type="submit" 
                            disabled={isLoading} 
                            className={`w-full font-montserrat font-bold py-3.5 lg:py-3 xl:py-4 rounded-xl text-[11px] xl:text-sm tracking-widest shadow-lg transform active:scale-95 transition-all duration-300 uppercase
                                ${isLoading 
                                    ? 'bg-gray-200 text-gray-400 cursor-not-allowed' 
                                    : 'bg-[#001e33] hover:bg-[#062c54] text-white'
                                }`}
            ...
            {/* ================= MITAD DERECHA: TARJETA DE PLANES ================= */}
            <div className="w-full lg:w-[55%] xl:w-7/12 p-0 lg:p-4 xl:p-6 flex">
                <div className="relative w-full h-full min-h-[500px] lg:min-h-full rounded-none lg:rounded-[2.5rem] overflow-hidden flex flex-col justify-center items-center px-6 sm:px-12 py-10 lg:py-6 xl:py-16 isolate">
                    ...
                    <div className="relative z-10 w-full max-w-3xl">
                        <h3 className="text-2xl lg:text-2xl xl:text-4xl font-bold mb-6 lg:mb-4 xl:mb-10 text-[#fcd34d] text-center font-montserrat flex items-center justify-center gap-4">
                            <SolutionOutlined className="text-[#fcd34d]" />
                            Planes de Asesoría Legal
                        </h3>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-4 xl:gap-8 w-full">
                            {/* Plan Mensual */}
                            <div className="bg-white p-5 lg:p-4 xl:p-8 rounded-[1.5rem] lg:rounded-[2rem] shadow-2xl flex flex-col h-full border border-gray-100">
                                <h4 className="text-lg lg:text-base xl:text-xl font-bold mb-1 text-[#001e33] text-center font-montserrat">Plan Mensual</h4>
                                <p className="text-[11px] lg:text-[10px] xl:text-[13px] text-gray-600 text-center mb-4 lg:mb-3 xl:mb-6 leading-tight">Ideal para empresas que requieren un respaldo jurídico preventivo.</p>
                                <ul className="space-y-2 lg:space-y-1.5 xl:space-y-4 text-[11px] lg:text-[11px] xl:text-[13px] text-[#001e33] font-roboto flex-grow">
                                    <li className="flex items-start gap-2 leading-tight"><CheckCircleOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Acompañamiento legal continuo en derecho laboral, comercial y Seguridad y Salud en el Trabajo.</li>
                                    <li className="flex items-start gap-2 leading-tight"><PhoneOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Consultas jurídicas ilimitadas por correo electrónico, teléfono, videollamada o Whatsapp.</li>
                                    <li className="flex items-start gap-2 leading-tight"><FileTextOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Revisión y elaboración de documentos legales.</li>
                                    <li className="flex items-start gap-2 leading-tight"><BellOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Alerta de vencimientos legales y gestión de riesgos jurídicos.</li>
                                    <li className="flex items-start gap-2 leading-tight"><TeamOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Capacitaciones jurídicas a tu equipo.</li>
                                </ul>
                            </div>

                            {/* Servicios por Evento */}
                            <div className="bg-white p-5 lg:p-4 xl:p-8 rounded-[1.5rem] lg:rounded-[2rem] shadow-2xl flex flex-col h-full border border-gray-100">
                                <h4 className="text-lg lg:text-base xl:text-xl font-bold mb-1 text-[#001e33] text-center font-montserrat">Servicios por Evento</h4>
                                <p className="text-[11px] lg:text-[10px] xl:text-[13px] text-gray-600 text-center mb-4 lg:mb-3 xl:mb-6 leading-tight">Para empresas que requieren apoyo jurídico puntual en casos específicos.</p>
                                <ul className="space-y-2 lg:space-y-1.5 xl:space-y-4 text-[11px] lg:text-[11px] xl:text-[13px] text-[#001e33] font-roboto flex-grow">
                                    <li className="flex items-start gap-2 leading-tight"><CheckCircleOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Asesoría especializada para un caso o situación puntual.</li>
                                    <li className="flex items-start gap-2 leading-tight"><ThunderboltOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Estrategias jurídicas enfocadas en resultados inmediatos.</li>
                                    <li className="flex items-start gap-2 leading-tight"><AuditOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Gestión en trámites, reclamaciones, auditorías o requerimientos.</li>
                                    <li className="flex items-start gap-2 leading-tight"><CheckCircleOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> No requiere compromiso mensual.</li>
                                    <li className="flex items-start gap-2 leading-tight"><ClockCircleOutlined className="mt-0.5 text-[#001e33] flex-shrink-0" /> Tiempo de respuesta prioritario según disponibilidad.</li>
                                </ul>
                            </div>
                        </div>

                        <div className="mt-8 lg:mt-6 xl:mt-14 flex justify-center">
                            <button
                                className="inline-flex items-center gap-3 bg-[#e6ce5a] text-[#001e33] font-bold font-montserrat py-3 lg:py-2.5 xl:py-4 px-8 lg:px-6 xl:px-10 rounded-xl shadow-xl hover:scale-105 transition-all duration-300 text-[11px] xl:text-sm tracking-tighter"
                            >
                                <CalendarOutlined className="text-lg xl:text-xl" />
                                <span>AGENDA TU PRIMERA CONSULTA</span>
                            </button>
                        </div>
                    </div>
